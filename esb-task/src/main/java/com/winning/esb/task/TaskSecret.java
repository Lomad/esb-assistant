...
@Component
public class TaskSecret implements ITask {
    private static final Logger logger = LoggerFactory.getLogger(TaskSecret.class);
    ...
    public void run() {
        //将授权信息同步到Esb
        if (SyncToEsbMark.getSyncSecret()) {
            try {
                releaseSecret();
                //关闭同步开关
                SyncToEsbMark.setSyncSecret(false);
            } catch (Exception ex) {
    ...
    public String releaseSecret() {
        ...
        middlewareService.releaseSecret(url);
        return null;
    }
